...
def create_group(req: GroupCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def update_group(group_id: int, req: GroupUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def list_all_devices(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def create_device(req: DeviceCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def update_device(device_id: int, req: DeviceUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    device = db.query(Device).filter(Device.id == device_id).first()
    if not device:
        raise HTTPException(status_code=404, detail="设备不存在")
    for k, v in req.model_dump(exclude_unset=True).items():
...
def list_tags(device_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return db.query(DeviceTag).filter(DeviceTag.device_id == device_id).order_by(DeviceTag.sort_order, DeviceTag.id).all()


@router.post("/tags", response_model=TagOut)
def create_tag(req: TagCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def update_tag(tag_id: int, req: TagUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def batch_create_tags(tags: List[TagCreate], db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    result = []
    for req in tags:
        tag = DeviceTag(**req.model_dump())
        db.add(tag)
...
def write_tag_value(device_id: int, req: WriteRequest, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def get_device_live_values(device_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
