...
def list_all_rules(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return db.query(AlarmRule).order_by(AlarmRule.id).all()


@router.post("/rules", response_model=AlarmRuleOut)
def create_rule(req: AlarmRuleCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def update_rule(rule_id: int, req: AlarmRuleUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def list_active_alarms(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def clear_alarm(record_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    record = db.query(AlarmRecord).filter(AlarmRecord.id == record_id).first()
    if not record:
        raise HTTPException(status_code=404, detail="报警记录不存在")
...
def get_alarm_stats(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
