...
def list_contacts(db: Session = Depends(get_db), _: User = Depends(require_permission("sms.read"))):
    return db.query(SmsContact).order_by(SmsContact.id).all()


@router.post("/contacts", response_model=SmsContactOut)
def create_contact(req: SmsContactCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
...
def update_contact(contact_id: int, req: SmsContactUpdate, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
...
def delete_contact(contact_id: int, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
    contact = db.query(SmsContact).filter(SmsContact.id == contact_id).first()
    if not contact:
        raise HTTPException(status_code=404, detail="联系人不存在")
    db.delete(contact)
...
def list_push_rules(db: Session = Depends(get_db), _: User = Depends(require_permission("sms.read"))):
    return db.query(SmsPushRule).order_by(SmsPushRule.id).all()


@router.post("/rules", response_model=SmsPushRuleOut)
def create_push_rule(req: SmsPushRuleCreate, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
...
def update_push_rule(rule_id: int, req: SmsPushRuleUpdate, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
...
def delete_push_rule(rule_id: int, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.write"))):
    rule = db.query(SmsPushRule).filter(SmsPushRule.id == rule_id).first()
    if not rule:
        raise HTTPException(status_code=404, detail="规则不存在")
    db.delete(rule)
...
@router.post("/test")
def test_sms(req: SmsTestRequest, db: Session = Depends(get_db), _: User = Depends(require_permission("sms.send"))):
